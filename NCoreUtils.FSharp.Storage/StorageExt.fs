...
type IStoragePath with
  [<MethodImpl(MethodImplOptions.AggressiveInlining)>]
  member this.AsyncGetParent () =
    let inline f cancellationToken = this.GetParentAsync cancellationToken
    Async.Adapt f

type IStorageContainer with
  [<MethodImpl(MethodImplOptions.AggressiveInlining)>]
  member this.AsyncGetContents () =
    this.GetContentsAsync () |> ofAsyncEnumerable
